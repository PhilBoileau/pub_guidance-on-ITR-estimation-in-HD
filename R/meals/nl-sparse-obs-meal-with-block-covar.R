...
cov_mat <- block_diag_cov_mat(500)
...
experiment <- create_experiment(name = "nl-sparse-obs-block-cov") %>%
  add_dgp(cont_out_dgp, name = "nl_sparse_obs_block_cov") %>%
  add_vary_across(
    .dgp = "nl_sparse_obs_block_cov",
    n = c(250, 500, 1000)
  ) %>%
