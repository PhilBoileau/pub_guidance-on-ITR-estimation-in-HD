...
experiment <- create_experiment(name = "nl-non-sparse-rct-identity-cov") %>%
  add_dgp(cont_out_dgp, name = "nl_non_sparse_rct_identity_cov") %>%
  add_vary_across(
    .dgp = "nl_non_sparse_rct_identity_cov",
    n = c(250, 500, 1000)
  ) %>%
