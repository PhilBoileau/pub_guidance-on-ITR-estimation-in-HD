...
experiment <- create_experiment(name = "lm-non-sparse-obs-identity-cov") %>%
  add_dgp(cont_out_dgp, name = "lm_non_sparse_obs_identity_cov") %>%
  add_vary_across(
    .dgp = "lm_non_sparse_obs_identity_cov",
    n = c(250, 500, 1000)
  ) %>%
